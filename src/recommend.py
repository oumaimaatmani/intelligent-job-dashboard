...
def build_and_save(corpus, out_dir: str = "../models/tfidf", max_features: int = 10000, ngram_range=(1, 2)) -> Tuple[TfidfVectorizer, object]:
    out = Path(out_dir)
...
def load_vectorizer_and_matrix(model_dir: str = "../models/tfidf") -> Tuple[TfidfVectorizer, object]:
    p = Path(model_dir)
...
def recommend(
    ...
    if extract_skills and skills_field in dfc.columns:
        try:
            dfc["_extracted_skills"] = dfc[skills_field].fillna("").astype(str).apply(
                lambda t: extract_skills_from_text(t, use_spacy=False)
            )
        except Exception as e:
            logger.debug("Skills extraction failed: %s", e)
    
    return dfc.sort_values("_similarity_score", ascending=False).head(top_n).reset_index(drop=True)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Build/load TF-IDF model and recommend jobs for a query")
    parser.add_argument("--data", type=str, default="../data/processed/job_listings_clean.csv")
    parser.add_argument("--build", action="store_true", help="Build and save vectorizer/matrix from data")
    parser.add_argument("--model-dir", type=str, default="../models/tfidf")
    ...
    if args.build:
        ...
        build_and_save(df.get("job_description", "").fillna("").astype(str).tolist(), out_dir=args.model_dir)
