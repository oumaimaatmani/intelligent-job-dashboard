...
from config.paths import CLEAN_CSV_PATH, RAW_CSV_PATH
...
def clean_job_data(input_path: str, output_path: str) -> None:
    ...
    print(
        f"[STATS] With salary: {df['has_salary'].sum()} ({df['has_salary'].mean() * 100:.1f}%)"
    )
    print(
        f"[STATS] With skills: {df['has_skills'].sum()} ({df['has_skills'].mean() * 100:.1f}%)"
    )
...
Examples:
  ...
  python clean_data.py --input data/raw/job_listings_raw.csv --output data/processed/job_listings_clean.csv
    ...
    parser.add_argument(
        "--input", type=str, default=str(RAW_CSV_PATH), help="Input raw CSV file path"
    )
    parser.add_argument(
        "--output",
        type=str,
