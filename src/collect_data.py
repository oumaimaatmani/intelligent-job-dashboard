...
from config.paths import RAW_CSV_PATH, SETTINGS_EXAMPLE_PATH, SETTINGS_PATH
...
def fetch_adzuna_jobs(
    query: str, locations: List[str], limit: int, cfg: Dict[str, Any]
) -> List[Dict[str, Any]]:
    results: List[Dict[str, Any]] = []
    app_id = os.getenv("ADZUNA_APP_ID") or cfg.get("apis", {}).get("adzuna", {}).get(
        "app_id"
    )
    app_key = os.getenv("ADZUNA_APP_KEY") or cfg.get("apis", {}).get("adzuna", {}).get(
        "app_key"
    )
    ...
    for loc in locations:
        ...
        while fetched < limit:
            ...
            try:
                ...
                print(
                    f"[INFO] Page {page}: {len(hits)} jobs collected ({fetched}/{limit})"
                )
                page += 1
                time.sleep(0.5)
...
APPEND MODE: New data is automatically added to existing file without overwriting!
    ...
    parser.add_argument(
        "--output", type=str, default=str(RAW_CSV_PATH), help="Output CSV file path"
    )
