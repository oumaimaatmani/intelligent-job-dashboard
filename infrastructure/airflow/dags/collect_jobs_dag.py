...
# Import centralized paths (after adding src to path)
...
def log_batch_run(
    ...
    try:
        ...
        conn.close()
        logger.info(
            f"Logged batch_run: {batch_id} | {stage} | {status} | {jobs_count} jobs"
        )
...
def stage_1_collect(**context):
    ...
    from collect_data import append_to_csv, fetch_adzuna_jobs, load_config, unify_schema
    ...
    try:
        ...
        raw_csv_path = str(PROCESSED_DIR / f"batch_{batch_id}_collected.csv")
        df = None
        rows_count = 0
        ...
        if cfg:
            ...
            try:
                ...
                logger.info(
                    f"[COLLECT] Fetching jobs: query='{query}', locations={locations}, limit={limit}"
                )
                ...
                logger.warning(
                    f"[COLLECT] API fetch failed: {api_error}. Will skip API collection."
                )
        else:
            logger.warning(
                "[COLLECT] No API configuration found. Skipping API collection."
            )

        # If no rows from API, use existing data or create empty
        if rows_count == 0:
            # Sample data is mounted at /data/raw in the container
            sample_data_path = str(RAW_CSV_PATH)
            ...
            if os.path.exists(sample_data_path):
                ...
                logger.info(
                    f"[COLLECT] Loaded {rows_count} jobs from sample data and saved to {raw_csv_path}"
                )
            else:
                logger.warning(
                    f"[COLLECT] Sample data not found at {sample_data_path}. Using empty dataset."
                )
                rows_count = 0
        ...
        logger.info(
            f"[COLLECT] Stage 1 completed in {execution_time}s - {rows_count} jobs collected"
        )
...
def stage_2_validate(**context):
    ...
    batch_id = context["task_instance"].xcom_pull(
        task_ids="collect_jobs", key="batch_id"
    )
    raw_csv_path = context["task_instance"].xcom_pull(
        task_ids="collect_jobs", key="raw_csv_path"
    )
    jobs_collected = context["task_instance"].xcom_pull(
        task_ids="collect_jobs", key="jobs_collected"
    )

    logger.info(
        f"[VALIDATE] Starting Stage 2 - batch_id: {batch_id}, jobs_collected: {jobs_collected}"
    )
    ...
    try:
        ...
        for idx, row in df.iterrows():
            ...
            if not pd.isna(row.get("salary_min")) and not pd.isna(
                row.get("salary_max")
            ):
                try:
...
def stage_3_enrich(**context):
    ...
    try:
        ...
        context["task_instance"].xcom_push(
            key="processed_csv_path", value=processed_csv_path
        )

        logger.info(
            f"[ENRICH] Stage 3 completed in {execution_time}s - {jobs_enriched} jobs enriched"
        )
...
def stage_4_vectorize(**context):
    ...
    processed_csv_path = context["task_instance"].xcom_pull(
        task_ids="enrich_jobs", key="processed_csv_path"
    )
    ...
    try:
        ...
        vectorizer, matrix = build_and_save(
            corpus, out_dir=str(MODELS_DIR), max_features=10000, ngram_range=(1, 2)
        )
        ...
        try:
            ...
            cache_vectorizer_and_matrix(
                vectorizer, matrix, model_version, ttl_seconds=604800
            )  # 7 days TTL
        ...
        logger.info(
            f"[VECTORIZE] Stage 4 completed in {execution_time}s - {jobs_vectorized} jobs vectorized"
        )
    ...
    try:
        ...
        for idx, row in df.iterrows():
            try:
                # Check if job exists
                cursor.execute(
                    "SELECT job_id FROM jobs WHERE job_id = %s", (row["job_id"],)
                )
                ...
                logger.error(
                    f"[EXPORT] Failed to insert/update job {row.get('job_id')}: {e}"
                )
                jobs_failed += 1
        ...
        conn.commit()
        logger.info(
            f"[EXPORT] Inserted: {jobs_inserted}, Updated: {jobs_updated}, Failed: {jobs_failed}"
        )
        ...
        logger.info(
            f"[EXPORT] Stage 5 completed in {execution_time}s - {jobs_inserted} inserted, {jobs_updated} updated"
        )
...
task_enrich = PythonOperator(
    task_id="enrich_jobs", python_callable=stage_3_enrich, provide_context=True, dag=dag
)
...
task_export = PythonOperator(
    task_id="export_jobs", python_callable=stage_5_export, provide_context=True, dag=dag
)
